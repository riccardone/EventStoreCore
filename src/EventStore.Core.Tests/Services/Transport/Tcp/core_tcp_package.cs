    ...
    public class core_tcp_package
    {
        ...
        public void should_throw_argument_null_exception_when_created_as_authorized_but_login_not_provided()
        {
            Assert.Throws<ArgumentNullException>(() =>
                new TcpPackage(TcpCommand.BadRequest, AmqpFlags.Authenticated, Guid.NewGuid(), null, "pa$$", new byte[] { 1, 2, 3 }));
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() =>
                new TcpPackage(TcpCommand.BadRequest, AmqpFlags.Authenticated, Guid.NewGuid(), "login", null, new byte[] { 1, 2, 3 }));
        }
        ...
        public void should_throw_argument_exception_when_created_as_not_authorized_but_login_is_provided()
        {
            Assert.Throws<ArgumentException>(() =>
                new TcpPackage(TcpCommand.BadRequest, AmqpFlags.None, Guid.NewGuid(), "login", null, new byte[] { 1, 2, 3 }));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() =>
                new TcpPackage(TcpCommand.BadRequest, AmqpFlags.None, Guid.NewGuid(), null, "pa$$", new byte[] { 1, 2, 3 }));
        }
        ...
        public void not_authorized_with_data_should_serialize_and_deserialize_correctly()
        {
            ...
            var refPkg = new TcpPackage(TcpCommand.BadRequest, AmqpFlags.None, corrId, null, null, new byte[] { 1, 2, 3});
            ...
            Assert.AreEqual(AmqpFlags.None, pkg.Flags);
        ...
        {
            var corrId = Guid.NewGuid();
            var refPkg = new TcpPackage(TcpCommand.BadRequest, AmqpFlags.None, corrId, null, null, new byte[0]);
            var bytes = refPkg.AsArraySegment();
        ...
        {
            var corrId = Guid.NewGuid();
            var refPkg = new TcpPackage(TcpCommand.BadRequest, AmqpFlags.Authenticated, corrId, "login", "pa$$", new byte[] { 1, 2, 3 });
            var bytes = refPkg.AsArraySegment();
        ...
        {
            var corrId = Guid.NewGuid();
            var refPkg = new TcpPackage(TcpCommand.BadRequest, AmqpFlags.Authenticated, corrId, "login", "pa$$", new byte[0]);
            var bytes = refPkg.AsArraySegment();
        ...
        {
            var pkg = new TcpPackage(TcpCommand.BadRequest, AmqpFlags.Authenticated, Guid.NewGuid(), new string('*', 256), "pa$$", new byte[] { 1, 2, 3 });
        ...
        {
            var pkg = new TcpPackage(TcpCommand.BadRequest, AmqpFlags.Authenticated, Guid.NewGuid(), "login", new string('*', 256), new byte[] { 1, 2, 3 });
            Assert.Throws<ArgumentException>(() => pkg.AsByteArray());
        }
